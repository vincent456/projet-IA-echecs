...
/**
 * classe Fenetre jeu servant ï¿½ representer la GUI du jeu d'echec, contient la
 * planche de jeu, les boutons debuter et reset, et le champ texte informant le
 * joueur sur le tour. Contient ï¿½galement deux jPanel contenant les pieces
 * mangï¿½es.
 *
 */
public class FenetreJeu extends JFrame {
	...
	/**
	 * Constructeur, appelle mï¿½thode JBInit
	 */
	public FenetreJeu() {
		try {
			jbInit();
	...
	/**
	 * initialise la surface de jeu. Crï¿½ï¿½ tout les ï¿½lements et initialise leur
	...
	private void jbInit() throws Exception {
		...
		for (int ligne = 0; ligne < 8; ligne++) {
			...
			for (int colonne = 0; colonne < 8; colonne++) {
				tab[colonne][ligne] = new JLabel(); // crï¿½ï¿½ation du JLabel
				...
				tab[colonne][ligne].addMouseListener(gest); // ajouter l'ï¿½ï¿½couteur aux
				if ((colonne + 1) % 2 == a)
	...
	private class GestionnaireEvenement extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent eve) {
			...
			if (eve.getSource() == boutonDebuter) {
				// initialise le champ texte, apelle la mï¿½thode dï¿½buter, et initialise toute
				// les
				// variables
				...
				// Je place les icï¿½nes des piï¿½ces sur leur cases respectives
				while (increment >= -1) {
			...
			// si on clique sur le bouton reset
				...
				// j'appelle la ï¿½thode RAZ
				RAZ();

			}

			else if (eve.getSource() instanceof JLabel) // donc on a cliquï¿½ sur un Label
			{
				for (int i = 0; i < 8; i++)
					// je dï¿½termine sur quelle Jlabel on a cliquï¿½
					for (int j = 0; j < 8; j++)
				...
				// si on a cliquï¿½ sur une case non vide et que le tampon n'est pas null
	...
	public boolean jouerUnCoup(Echiquier e, int colonneClic, int ligneClic, String couleurControle) {
		if ((e.getCase(colonneClic, ligneClic).getPiece() != null | pieceTampon != null)) {
			...
			if (pieceTampon == null) {
				// si c'est au tour de la couleur de controle ï¿½ jouer
				if (e.getCase(colonneClic, ligneClic).getPiece().getCouleur().equals(couleurControle)) {
					// J'initialise la piece tampon a la piece sur laquelle on a cliquï¿½
				...
				// je crï¿½ï¿½ un dï¿½placement
				Deplacement deplacement = new Deplacement(temp, new Position(colonneClic, ligneClic));
				// je vï¿½rifie si le dï¿½placement est valide, si le chemin est possible et si
				// il
				...
				if ((pieceTampon.estValide(deplacement) && e.cheminPossible(deplacement))
					...
					// je crï¿½ï¿½ un jLabel avec l'icï¿½ne de la piï¿½ce manger
					...
					/*
					 * je vï¿½rifie si la piï¿½ce manger est un roi, si oui le jeu est terminï¿½ et
					...
					if (e.getCase(colonneClic, ligneClic).getPiece() instanceof Roi) {
						if (JOptionPane.showConfirmDialog(null,
								"Fï¿½licitation vous avez gagnï¿½ ! Dï¿½sirez-vous jouer de nouveau ?\n", "Mine !",
					...
					} else// si on dï¿½pose la piece sur une case vide
					{
						// on met le tampon sur la case vide et on vide le tampon aprï¿½s
	...
	public static void main(String[] args) {
		...
		j.setDefaultCloseOperation(EXIT_ON_CLOSE); // ferme le processus associï¿½
		ia = new IA(j);
	}
}